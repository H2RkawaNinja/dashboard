...
function initDealerMap() {
    ...
    // Drag to pan
    mapWrap.removeEventListener('mousedown', _dealerDragStart);
    mapWrap.addEventListener('mousedown', _dealerDragStart);
    document.removeEventListener('mousemove', _dealerDragMove);
    document.addEventListener('mousemove', _dealerDragMove);
    document.removeEventListener('mouseup', _dealerDragEnd);
    document.addEventListener('mouseup', _dealerDragEnd);
}
...
function _dealerDragStart(e) {
    if (e.button !== 0) return;
    const wrap = document.getElementById('dealer-map-wrap');
    if (!wrap) return;
    dealerDragActive = true;
    dealerDragMoved  = false;
    dealerDragSX = e.clientX;
    dealerDragSY = e.clientY;
    dealerDragSL = wrap.scrollLeft;
    dealerDragST = wrap.scrollTop;
    wrap.style.cursor = 'grabbing';
    e.preventDefault();
}

function _dealerDragMove(e) {
    if (!dealerDragActive) return;
    const dx = e.clientX - dealerDragSX;
    const dy = e.clientY - dealerDragSY;
    if (!dealerDragMoved && (Math.abs(dx) > 4 || Math.abs(dy) > 4)) dealerDragMoved = true;
    if (!dealerDragMoved) return;
    const wrap = document.getElementById('dealer-map-wrap');
    if (!wrap) return;
    wrap.scrollLeft = dealerDragSL - dx;
    wrap.scrollTop  = dealerDragST - dy;
}

function _dealerDragEnd() {
    if (!dealerDragActive) return;
    dealerDragActive = false;
    const wrap = document.getElementById('dealer-map-wrap');
    if (wrap) wrap.style.cursor = '';
}

function handleDealerMapClick(e) {
    if (dealerDragMoved) { dealerDragMoved = false; return; }
    if (!dealerPlaceMode) return;
    const mapInner = document.getElementById('dealer-map-inner');
