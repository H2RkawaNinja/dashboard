...
let notesData = { sheets: [{ name: 'Tabelle 1', cols: NOTES_COLS, data: [] }] };
    ...
    try {
        ...
        try {
            ...
            if (parsed && parsed.sheets) {
                // Migriere alte Sheets die noch keine cols-Struktur haben
                parsed.sheets.forEach(s => {
                    if (!s.data) {
                        s.data = (s.rows || []).map(r => {
                            const cols = r.cols || [r.thema || '', r.details || '', r.priority || ''];
                            return cols;
                        });
                        s.cols = NOTES_COLS;
                        delete s.rows;
                ...
                notesData = { sheets: [{ name: 'Tabelle 1', cols: NOTES_COLS, data: [[data.notes.replace(/<[^>]+>/g,''), '', '', '', '', '']] }] };
            } else {
                notesData = { sheets: [{ name: 'Tabelle 1', cols: NOTES_COLS, data: [] }] };
            }
        } catch(e) {
            notesData = { sheets: [{ name: 'Tabelle 1', cols: NOTES_COLS, data: [] }] };
        }
...
function notesCurrentSheet() {
    return notesData.sheets[activeSheetIdx];
}

function notesGetCell(r, c) {
    const sheet = notesCurrentSheet();
    return (sheet.data[r] && sheet.data[r][c] != null) ? sheet.data[r][c] : '';
}

function notesSetCell(r, c, val) {
    const sheet = notesCurrentSheet();
...
function notesRowCount() {
    const sheet = notesCurrentSheet();
    return Math.max(NOTES_ROWS, sheet.data.length + 5);
}

function notesColCount() {
...
function notesRenderTabs() {
    ...
    container.innerHTML = notesData.sheets.map((sheet, i) => `
        <div class="nst-tab ${i === activeSheetIdx ? 'active' : ''}" data-idx="${i}">
            <span class="nst-label" onclick="notesSwitchSheet(${i})" ondblclick="notesRenameSheet(${i})">${sheet.name}</span>
            ${notesData.sheets.length > 1 ? `<span class="nst-close" onclick="notesDeleteSheet(${i})">×</span>` : ''}
        </div>
    `).join('');
}

function notesRenderGrid() {
    ...
    for (let c = 0; c < cols; c++) {
        colHeader += `<div class="nsg-col-head" data-col="${c}">${colLetter(c)}</div>`;
    }
    ...
    wrap.querySelectorAll('.nsg-cell').forEach(cell => {
        cell.addEventListener('click', notesOnCellClick);
        cell.addEventListener('keydown', notesOnCellKey);
...
function notesUpdateCellStyles() {
    const wrap = document.getElementById('notes-grid-wrap');
    if (!wrap) return;
    wrap.querySelectorAll('.nsg-cell.selected,.nsg-cell.nsg-in-range,.nsg-cell.nsg-clipboard').forEach(c => {
    ...
    wrap.querySelectorAll('.nsg-cell').forEach(cell => {
        ...
        if (r >= r1 && r <= r2 && c >= c1 && c <= c2) {
            if (!isFullRow && notesSelectedCell && r === notesSelectedCell.r && c === notesSelectedCell.c) {
                ...
                cell.classList.add('nsg-in-range');
            }
        }
        if (notesClipboard && notesClipboard.srcRange && notesClipboard.srcSheet === activeSheetIdx) {
            const sr = notesClipboard.srcRange;
            if (r >= sr.r1 && r <= sr.r2 && c >= sr.c1 && c <= sr.c2) cell.classList.add('nsg-clipboard');
        }
    });
}
...
function notesSelectCell(cell) {
    notesSelectedCell = { r: +cell.dataset.r, c: +cell.dataset.c };
    notesSelRange = { r1: notesSelectedCell.r, c1: notesSelectedCell.c, r2: notesSelectedCell.r, c2: notesSelectedCell.c };
    notesUpdateCellStyles();
    cell.focus();
    const bar = document.getElementById('notes-formula-bar');
    if (bar) bar.value = notesGetCell(notesSelectedCell.r, notesSelectedCell.c);
    notesUpdateToolbar();
}

function notesEditCell(cell) {
...
function notesOnCellBlur(e) {
    const cell = e.currentTarget;
    if (cell.contentEditable === 'true') {
        const r = +cell.dataset.r, c = +cell.dataset.c;
        const val = cell.innerText.trim();
        notesSetCell(r, c, val);
        cell.contentEditable = 'false';
        cell.classList.remove('editing');
        cell.style.cssText = notesBuildCellStyle(r, c);
        cell.innerHTML = val ? escapeHtml(val) : '';
        const bar = document.getElementById('notes-formula-bar');
        if (bar && notesSelectedCell && notesSelectedCell.r === r && notesSelectedCell.c === c) bar.value = val;
    }
}

function notesOnCellKey(e) {
    const cell = e.currentTarget;
    ...
    if (!isEditing) {
        if (e.key === 'Enter' || e.key === 'F2') { e.preventDefault(); notesEditCell(cell); return; }
        if ((e.key === 'Delete' || e.key === 'Backspace') && !e.shiftKey) {
            ...
            if (notesSelRange) {
                ...
                for (let rr=r1;rr<=r2;rr++) for (let cc=c1;cc<=c2;cc++) {
                    notesSetCell(rr,cc,'');
                    const tc = document.querySelector(`#notes-grid-wrap .nsg-cell[data-r="${rr}"][data-c="${cc}"]`);
                    if (tc) tc.innerHTML='';
                }
            }
            const b = document.getElementById('notes-formula-bar'); if(b) b.value='';
            return;
        }
        if (e.shiftKey && e.key === 'ArrowRight') { e.preventDefault(); notesExtendSel(r, c+1); return; }
        if (e.shiftKey && e.key === 'ArrowLeft')  { e.preventDefault(); notesExtendSel(r, c-1); return; }
        if (e.shiftKey && e.key === 'ArrowDown')  { e.preventDefault(); notesExtendSel(r+1, c); return; }
        if (e.shiftKey && e.key === 'ArrowUp')    { e.preventDefault(); notesExtendSel(r-1, c); return; }
        if (e.shiftKey && e.key === 'Tab') { e.preventDefault(); notesMoveTo(r, c-1); return; }
        if (e.key === 'ArrowRight' || e.key === 'Tab') { e.preventDefault(); notesMoveTo(r, c+1); return; }
        ...
        if (e.key === 'Tab') { e.preventDefault(); cell.blur(); notesMoveTo(r, c+(e.shiftKey?-1:1)); return; }
    }
}

function notesExtendSel(r2, c2) {
    if (!notesSelectedCell) return;
    r2 = Math.max(0, Math.min(notesRowCount()-1, r2));
    c2 = Math.max(0, Math.min(notesColCount()-1, c2));
    notesSelRange = { r1: notesSelectedCell.r, c1: notesSelectedCell.c, r2, c2 };
    notesUpdateCellStyles();
    const wrap = document.getElementById('notes-grid-wrap');
    if (wrap) { const t = wrap.querySelector(`.nsg-cell[data-r="${r2}"][data-c="${c2}"]`); if(t) t.scrollIntoView({block:'nearest',inline:'nearest'}); }
}

function notesCopy(cut = false) {
    if (!notesSelRange) return;
    const { r1, c1, r2, c2 } = normalizeRange(notesSelRange);
    const data = [], styles = [];
    for (let r = r1; r <= r2; r++) {
        const row = [], srow = [];
        for (let c = c1; c <= c2; c++) { row.push(notesGetCell(r,c)); srow.push({...notesGetCellStyle(r,c)}); }
        data.push(row); styles.push(srow);
    }
    notesClipboard = { data, styles, srcRange: normalizeRange(notesSelRange), srcSheet: activeSheetIdx, cut };
    if (cut) {
        for (let r=r1;r<=r2;r++) for (let c=c1;c<=c2;c++) {
            notesSetCell(r,c,'');
            const cell = document.querySelector(`#notes-grid-wrap .nsg-cell[data-r="${r}"][data-c="${c}"]`);
...
function notesMoveTo(r, c) {
    const rows = notesRowCount(), cols = notesColCount();
    r = Math.max(0, Math.min(rows - 1, r));
    c = Math.max(0, Math.min(cols - 1, c));
    const wrap = document.getElementById('notes-grid-wrap');
    if (!wrap) return;
    const target = wrap.querySelector(`.nsg-cell[data-r="${r}"][data-c="${c}"]`);
    if (target) { notesSelectCell(target); target.scrollIntoView({ block: 'nearest', inline: 'nearest' }); }
}

// Formelleiste Eingabe
function notesFormulaInput(val) {
    if (!notesSelectedCell) return;
    const { r, c } = notesSelectedCell;
    notesSetCell(r, c, val);
    const wrap = document.getElementById('notes-grid-wrap');
    if (!wrap) return;
    const cell = wrap.querySelector(`.nsg-cell[data-r="${r}"][data-c="${c}"]`);
    if (cell) cell.innerHTML = val ? escapeHtml(val) : '';
}

// ---- Sheets ----
function notesSwitchSheet(idx) {
    if (idx === activeSheetIdx) return;
    activeSheetIdx = idx;
    notesSelectedCell = null; notesSelRange = null;
    notesRender();
}

function addNotesSheet() {
    const newIdx = notesData.sheets.length;
    notesData.sheets.push({ name: `Tabelle ${newIdx + 1}`, cols: NOTES_COLS, data: [] });
    activeSheetIdx = newIdx;
    notesRender();
}

function notesDeleteSheet(idx) {
...
function notesRenameSheet(idx) {
    const tab = document.querySelector(`.nst-tab[data-idx="${idx}"] .nst-label`);
    if (!tab) return;
    const cur = notesData.sheets[idx].name;
    tab.contentEditable = 'true'; tab.focus();
    const rng = document.createRange(); rng.selectNodeContents(tab);
    const sel = window.getSelection(); sel.removeAllRanges(); sel.addRange(rng);
    const done = () => { tab.contentEditable='false'; notesData.sheets[idx].name = tab.innerText.trim()||cur; tab.removeEventListener('blur',done); tab.removeEventListener('keydown',onk); };
    const onk = (e) => { if(e.key==='Enter'){e.preventDefault();done();} if(e.key==='Escape'){tab.innerText=cur;done();} };
...
// Legacy-Stubs (werden nicht mehr gebraucht, aber falls irgendwo referenziert)
function renderNotesTable() {}
function collectNotesRows() { return []; }
...
function showPrivatePurchaseModal(fromCart = false) {
    ...
    const cartSummary = document.getElementById('private-cart-summary');
    const singleSection = document.getElementById('private-single-section');

    if (fromCart && shoppingCart.length > 0) {
        // Warenkorb-Modus: Artikel aus dem Warenkorb anzeigen
        cartSummary.style.display = 'block';
        singleSection.style.display = 'none';

        const total = shoppingCart.reduce((sum, item) => sum + item.total, 0);
        document.getElementById('private-cart-total').textContent = total.toFixed(2);
        document.getElementById('private-cart-items').innerHTML = shoppingCart.map(item => `
            <div style="display:flex; justify-content:space-between; align-items:center; padding: 0.4rem 0; border-bottom: 1px solid var(--border);">
                <div>
                    ...
                    <span style="color:var(--text-secondary); font-size:0.85rem;"> · ${item.quantity}x @ $${item.unitPrice.toFixed(2)}</span>
                </div>
...
if (privatePurchaseForm) {
    privatePurchaseForm.addEventListener('submit', (e) => {
        e.preventDefault();
        const cartSummaryVisible = document.getElementById('private-cart-summary').style.display !== 'none';

        let summary;
        if (cartSummaryVisible && shoppingCart.length > 0) {
            const total = shoppingCart.reduce((sum, item) => sum + item.total, 0);
            const count = shoppingCart.length;
            summary = `${count} Artikel für $${total.toFixed(2)} — privat gekauft`;
            shoppingCart = [];
            updateCartDisplay();
        } else {
            ...
            summary = `${qty}x ${itemName} für $${total} — privat gekauft`;
        }
...
async function loadDealerSpots() {
    try {
        const res = await fetch(`${API_URL}/dealer-spots`, { credentials: 'include' });
        if (!res.ok) { showToast('Fehler beim Laden der Dealer', 'error'); return; }
        const data = await res.json();
...
function renderDealerSpots(filter) {
    ...
    pinsLayer.innerHTML = '';
    document.getElementById('dealer-stat-total').textContent = dealerSpotsData.length;
...
function createDealerPin(spot) {
    ...
    pin.addEventListener('click', (e) => {
        if (dealerPlaceMode) return;
        e.stopPropagation();
        openDealerSpotPopup(spot.id, e);
    });
    return pin;
}

function toggleDealerPlaceMode() {
    dealerPlaceMode = !dealerPlaceMode;
    const btn      = document.getElementById('dealer-place-btn');
    const hint     = document.getElementById('dealer-cursor-hint');
    const mapInner = document.getElementById('dealer-map-inner');
    if (dealerPlaceMode) {
...
function handleDealerMapClick(e) {
    if (!dealerPlaceMode) return;
    const mapInner = document.getElementById('dealer-map-inner');
    const rect = mapInner.getBoundingClientRect();
    const x = Math.max(0, Math.min(100, ((e.clientX - rect.left) / rect.width)  * 100));
    const y = Math.max(0, Math.min(100, ((e.clientY - rect.top)  / rect.height) * 100));
    dealerPendingPos = { x: parseFloat(x.toFixed(3)), y: parseFloat(y.toFixed(3)) };
    if (dealerPlaceMode) toggleDealerPlaceMode();
    showDealerNewModal();
}
...
function closeDealerNewModal() {
    document.getElementById('dealer-new-spot-modal').style.display = 'none';
    document.getElementById('modal-overlay').style.display = 'none';
    dealerPendingPos = null;
}
...
async function saveDealerNewSpot(e) {
    ...
    try {
        const res  = await fetch(`${API_URL}/dealer-spots`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
...
function showDealerEditModal() {
    ...
    setTimeout(() => document.getElementById('edit-dealer-name').focus(), 50);
}

function closeDealerEditModal() {
    document.getElementById('dealer-edit-spot-modal').style.display = 'none';
    document.getElementById('modal-overlay').style.display = 'none';
}

async function saveDealerEditSpot(e) {
    e.preventDefault();
    const id   = document.getElementById('edit-dealer-id').value;
    const name = document.getElementById('edit-dealer-name').value.trim();
    const desc = document.getElementById('edit-dealer-desc').value.trim();
    try {
        const res  = await fetch(`${API_URL}/dealer-spots/${id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
...
function openDealerSpotPopup(spotId, event) {
    const spot = dealerSpotsData.find(s => s.id === spotId);
    if (!spot) return;
    dealerSelectedSpotId = spotId;
    const popup = document.getElementById('dealer-spot-popup');
    ...
    const pw = 280, ph = 120;
    let px = event.clientX + 15, py = event.clientY - 50;
    if (px + pw > window.innerWidth  - 10) px = event.clientX - pw - 15;
    if (py < 10) py = 10;
    ...
    document.getElementById(`dealer-list-item-${spotId}`)?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}
...
function clickDealerSpotFromList(spotId) {
    ...
    if (mapWrap && mapImg && mapImg.offsetWidth > 0) {
        const scrollX = (spot.x_pos / 100) * mapImg.offsetWidth  - mapWrap.offsetWidth  / 2;
...
function renderDealerColorPalette(containerId, selected) {
    ...
    if (!container) return;
    container.innerHTML = DEALER_COLORS.map(color => `
        <button type="button"
                class="spot-color-btn ${color === selected ? 'active' : ''}"
                style="background:${color};${color === '#ffffff' ? 'border:2px solid #555;' : ''}"
                onclick="selectDealerColor('${color}','${containerId}')">
            ${color === selected ? `<i class="fas fa-check" style="font-size:0.65em;color:${color==='#ffffff'?'#000':'#fff'}"></i>` : ''}
        </button>
    `).join('');
}

function selectDealerColor(color, containerId) {
    dealerSelectedColor = color;
    renderDealerColorPalette(containerId, color);
}

// Close dealer popup when clicking outside
document.addEventListener('click', (e) => {
    const popup = document.getElementById('dealer-spot-popup');
    if (popup && popup.style.display === 'block') {
        if (!popup.contains(e.target) && !e.target.closest('.dealer-pin') && !e.target.closest('.dealer-spot-item')) {
            closeDealerPopup();
        }
    }
});
...
async function loadDealerSpots() {
    try {
        const res = await fetch(`${API_URL}/dealer-spots`, { credentials: 'include' });
        if (!res.ok) { showToast('Fehler beim Laden der Dealer-Spots', 'error'); return; }
        const data = await res.json();
        dealerSpotsData = data.spots || [];
        renderDealerSpots();
        initDealerMap();
...
function initDealerMap() {
    const mapInner = document.getElementById('dealer-map-inner');
    if (!mapInner) return;
    // Re-attach click handler (remove old first)
    mapInner.removeEventListener('click', handleDealerMapClick);
    mapInner.addEventListener('click', handleDealerMapClick);
    // Show/hide admin controls
    const canManage = currentUser && currentUser.can_add_members;
    document.querySelectorAll('.dealer-manage-only').forEach(el => {
        el.style.display = canManage ? '' : 'none';
    });
}
...
function renderDealerSpots() {
    ...
    const total = dealerSpotsData.length;
    const taken = dealerSpotsData.filter(s => s.assigned_member_id).length;
    const free  = total - taken;
    document.getElementById('dealer-stat-total').textContent = total;
    document.getElementById('dealer-stat-free').textContent  = free;
    document.getElementById('dealer-stat-taken').textContent = taken;

    const emptyState = document.getElementById('dealer-empty-state');

    if (total === 0) {
        if (emptyState) emptyState.style.display = '';
        listEl.innerHTML = `<div class="empty-state" id="dealer-empty-state">
            <i class="fas fa-map-pin"></i>
            <p>Noch keine Spots eingetragen</p>
            <small>Admins können Spots per Klick auf die Karte hinzufügen</small>
        </div>`;
        return;
    }
    ...
    // Sidebar list
    listEl.innerHTML = dealerSpotsData.map(spot => `
        <div class="dealer-spot-item ${spot.assigned_member_id ? 'taken' : 'free'}"
             id="dealer-list-item-${spot.id}"
             onclick="clickDealerSpotFromList(${spot.id})">
            <div class="dealer-spot-item-dot" style="background:${spot.color || '#ef4444'};"></div>
            <div class="dealer-spot-item-info">
                <div class="dealer-spot-item-name">${escapeHtml(spot.name)}</div>
                <div class="dealer-spot-item-sub">
                    ${spot.assigned_member_name
                        ? `<i class="fas fa-user"></i> ${escapeHtml(spot.assigned_member_name)}`
                        : '<span style="color:var(--success)"><i class="fas fa-circle" style="font-size:0.55em;"></i> Frei</span>'}
                </div>
            </div>
            <span class="dealer-spot-badge ${spot.assigned_member_id ? 'badge-taken' : 'badge-free'}">
                ${spot.assigned_member_id ? 'Belegt' : 'Frei'}
            </span>
        </div>
    `).join('');
}
...
function createDealerPin(spot) {
    const pin = document.createElement('div');
    pin.className = 'dealer-pin' + (spot.assigned_member_id ? ' pin-taken' : '');
    pin.id = `dealer-pin-${spot.id}`;
    pin.style.left = spot.x_pos + '%';
    pin.style.top  = spot.y_pos + '%';
    pin.innerHTML = `
    ...
    `;
    pin.addEventListener('click', (e) => {
        if (dealerPlaceMode) return; // Let map handle clicks in place mode
        e.stopPropagation();
        openDealerSpotPopup(spot.id, e);
    });
    return pin;
...
function handleDealerMapClick(e) {
    ...
    const y = Math.max(0, Math.min(100, ((e.clientY - rect.top)  / rect.height) * 100));
    dealerPendingPos = { x: parseFloat(x.toFixed(3)), y: parseFloat(y.toFixed(3)) };
    if (dealerPlaceMode) toggleDealerPlaceMode(); // exit place mode
    showDealerNewModal();
}
...
async function saveDealerNewSpot(e) {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) {
            showToast('Dealer-Spot erstellt!', 'success');
            closeDealerNewModal();
            loadDealerSpots();
        } else {
            showToast(data.error || 'Fehler', 'error');
...
function openDealerSpotPopup(spotId, event) {
    ...
    const assignEl = document.getElementById('dealer-popup-assignment');
    if (spot.assigned_member_id) {
        assignEl.innerHTML = `<div class="popup-assigned-row">
            <i class="fas fa-user-circle" style="color:var(--primary)"></i>
            <strong>${escapeHtml(spot.assigned_member_name)}</strong>
            <span class="rank-pill">${escapeHtml(spot.assigned_member_rank || '')}</span>
        </div>`;
    } else {
        assignEl.innerHTML = `<div class="popup-free-row"><i class="fas fa-circle" style="color:var(--success);font-size:0.6em;"></i> Freier Spot</div>`;
    }

    const isAssignedToMe = spot.assigned_member_id === currentUser.id;
    const isTaken = !!spot.assigned_member_id;
    document.getElementById('dealer-popup-register-btn').style.display   = !isTaken ? '' : 'none';
    document.getElementById('dealer-popup-unregister-btn').style.display  = isAssignedToMe ? '' : 'none';
    const canManage = currentUser && currentUser.can_add_members;
    document.getElementById('dealer-popup-edit-btn').style.display   = canManage ? '' : 'none';
    document.getElementById('dealer-popup-delete-btn').style.display = canManage ? '' : 'none';

    // Position popup near click
    popup.style.display = 'block';
    const pw = 290, ph = 180;
    let px = event.clientX + 15, py = event.clientY - 60;
    if (px + pw > window.innerWidth  - 10) px = event.clientX - pw - 15;
    if (py < 10) py = 10;
    if (py + ph > window.innerHeight - 10) py = window.innerHeight - ph - 10;
    popup.style.left = px + 'px';
...
function clickDealerSpotFromList(spotId) {
    const spot = dealerSpotsData.find(s => s.id === spotId);
    if (!spot) return;
    // Scroll the map to the pin
    const mapWrap = document.getElementById('dealer-map-wrap');
    if (mapWrap) {
        const mapImg = document.getElementById('dealer-map-img');
        const w = mapImg ? mapImg.offsetWidth  : mapWrap.offsetWidth;
        const h = mapImg ? mapImg.offsetHeight : mapWrap.offsetHeight;
        const scrollX = (spot.x_pos / 100) * w - mapWrap.offsetWidth  / 2;
        const scrollY = (spot.y_pos / 100) * h - mapWrap.offsetHeight / 2;
        mapWrap.scrollTo({ left: Math.max(0, scrollX), top: Math.max(0, scrollY), behavior: 'smooth' });
    }
    // Show popup near the pin
    const pin = document.getElementById(`dealer-pin-${spotId}`);
    if (pin) {
        const rect = pin.getBoundingClientRect();
        openDealerSpotPopup(spotId, { clientX: rect.left + rect.width / 2, clientY: rect.top });
...
async function dealerRegisterSelf() {
    if (!dealerSelectedSpotId) return;
    try {
        const res  = await fetch(`${API_URL}/dealer-spots/${dealerSelectedSpotId}/assign`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            credentials: 'include', body: JSON.stringify({ member_id: currentUser.id })
        });
        const data = await res.json();
        if (data.success) { showToast('Du hast dich für diesen Spot angemeldet!', 'success'); closeDealerPopup(); loadDealerSpots(); }
        else showToast(data.error || 'Fehler', 'error');
    } catch { showToast('Verbindungsfehler', 'error'); }
}

async function dealerUnregisterSelf() {
    if (!dealerSelectedSpotId) return;
    try {
        const res  = await fetch(`${API_URL}/dealer-spots/${dealerSelectedSpotId}/assign`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            credentials: 'include', body: JSON.stringify({ member_id: null })
        });
        const data = await res.json();
        if (data.success) { showToast('Du hast dich abgemeldet.', 'info'); closeDealerPopup(); loadDealerSpots(); }
        else showToast(data.error || 'Fehler', 'error');
    } catch { showToast('Verbindungsfehler', 'error'); }
}
...
async function confirmDeleteDealerSpot() {
    ...
    try {
        ...
        const data = await res.json();
        if (data.success) { showToast('Spot gelöscht.', 'success'); closeDealerPopup(); loadDealerSpots(); }
        else showToast(data.error || 'Fehler', 'error');
    } catch { showToast('Verbindungsfehler', 'error'); }
}
...
function renderDealerColorPalette(containerId, selected) {
    ...
    container.innerHTML = DEALER_COLORS.map(color => {
        ...
        const checkIcon = isSelected ? '<i class="fas fa-check" style="font-size:0.65em;"></i>' : '';
        const border = color === '#ffffff' ? 'border:2px solid #555;' : '';
        return `<button type="button"
                class="spot-color-btn ${isSelected ? 'active' : ''}"
